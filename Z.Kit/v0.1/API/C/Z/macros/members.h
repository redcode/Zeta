...
#if Z_COMPILER_HAS_VARIABLE(BIT_FIELD_ORDER)

#	define Z_BIT_FIELD(bits, member_count)			       \
		Z_TERNARY_INSERT(Z_COMPILER_VARIABLE(BIT_FIELD_ORDER)) \
			(Z_MEMBERIZE_, member_count, REVERSED_,)

#else

#	define Z_BIT_FIELD(bits, member_count)					   \
		Z_TERNARY_INSERT(Z_COMPILER_VARIABLE(BIT_FIELD_ORDER_##bits##BIT)) \
			(Z_MEMBERIZE_, member_count, REVERSED_,)
