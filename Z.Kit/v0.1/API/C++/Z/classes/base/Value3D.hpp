...
template <class T> struct Zeta::Value3D : public ZNumberType(Z3D, T) {

	typedef typename ZNumberType(Z3D, T) Base;
	typedef typename ZNumberType(Z3D, T) Super;
	typedef		 T		     Value;
...
/*	static Z_INLINE_MEMBER Value2D minimum(const Value2D &a, const Value2D &b)
	...
	Z_INLINE_MEMBER operator Boolean() const {return this->x != T(0) || this->y != T(0) || this->z != T(0);}
	Z_INLINE_MEMBER operator Base	() const {return *z_base;}

	Z_INLINE_MEMBER Boolean operator ==(const Value3D &value) const {return this->x == value.x && this->y == value.y && this->z == value.z;}
	Z_INLINE_MEMBER Boolean operator !=(const Value3D &value) const {return this->x != value.x || this->y != value.y || this->z != value.z;}
	Z_INLINE_MEMBER Boolean operator <=(const Value3D &value) const {return this->x <= value.x && this->y <= value.y;}
	Z_INLINE_MEMBER Boolean operator >=(const Value3D &value) const {return this->x >= value.x && this->y >= value.y;}
	Z_INLINE_MEMBER Boolean operator  >(const Value3D &value) const {return this->x >  value.x && this->y >  value.y;}
	Z_INLINE_MEMBER Boolean operator  <(const Value3D &value) const {return this->x <  value.x && this->y <  value.y;}
