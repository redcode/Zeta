...
namespace Zeta {template <class T> struct Rectangle {
	...
	typedef typename ZTypeSizedReal(ZRectangle, T) Base;
	...
	Z_CT_MEMBER(CPP11) Rectangle(const typename ZTypeSizedReal(ZAABR, T) &aabr)   : point(aabr.a), size(aabr.b - aabr.a) {}
...
#	endif
	...
	Z_CT_MEMBER(CPP11) Boolean contains(const typename ZTypeSizedReal(ZAABR, T) &aabr) const
		{
	...
	Z_CT_MEMBER(CPP11) Boolean contains(const typename ZTypeSizedReal(ZCircle, T) &circle) const
		{
