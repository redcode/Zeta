...
#include <Z/traits/SelectType.hpp>
	...
	struct Zeta::Template {
		...
		template <class C, class... A> struct Prepend;

		template <template <class...> class C, class... A, class... T>
		struct Prepend<C<A...>, T...> {
			typedef C<T..., A...> type;
		};

		template <class C, class... T> struct Append;
		...
		struct Remove<C<A0, A...>, I> {
			typedef typename Prepend<typename Remove<C<A...>, I - 1>::type, A0>::type type;
		};
...
#		if Z_LANGUAGE_HAS(CPP, TEMPLATE_ALIAS)
			...
			using prepend_type = typename Prepend<C, T...>::type;

			template <class C, class... T>
			using append_type = typename Append<C, T...>::type;
