...
#ifndef _Z_classes_AABB_HPP_
#define _Z_classes_AABB_HPP_

#include <Z/classes/Line3D.hpp>
...
#	define Z_THIS ((AABB *)this)
	...
	struct Part<AABB, T, Z_NUMBER_SET_R> {
		...
		Z_CT(CPP11) Zeta::XYZ<T> absolute_point_to_unit(const Zeta::XYZ<T> &point)
		...
		Z_CT(CPP11) Zeta::XYZ<T> unit_point_to_absolute(Zeta::XYZ<T> point)
...
namespace Zeta {template <class T> struct AABB : Line3D<T>, Partials::Rectangle::Part<AABB<T>, T, Type<T>::number_set> {
	...
	Z_CT(CPP11) AABR(const XY<T> &a, const XY<T> &b) : Line2D<T>(a, b)		   {}
	Z_CT(CPP11) AABR(const XY<T> &a, T b_x, T b_y)	   : Line2D<T>(a, b_x, b_y)	   {}
	Z_CT(CPP11) AABR(const XY<T> &a, T b_xy)		   : Line2D<T>(a, b_xy)		   {}
	Z_CT(CPP11) AABR(T a_x, T a_y, const XY<T> &b)	   : Line2D<T>(a_x, a_y, b)	   {}
	Z_CT(CPP11) AABR(T a_x, T a_y, T b_x, T b_y)		   : Line2D<T>(a_x, a_y, b_x, b_y) {}
	Z_CT(CPP11) AABR(T a_xy, T b_x, T b_y)			   : Line2D<T>(a_xy, b_x, b_y)	   {}
	Z_CT(CPP11) AABR(T a_xy, const XY<T> &b)		   : Line2D<T>(a_xy, b)		   {}
