...
#ifndef _Z_classes_Line3D_HPP_
#define _Z_classes_Line3D_HPP_
...
Z_INLINE Line3D() Z_DEFAULTED({})								\
												     \
Z_CT(CPP11) Line3D(const XYZ<T> &a, const XYZ<T> &b)	     : a(a),		 b(b)		  {} \
Z_CT(CPP11) Line3D(const XYZ<T> &a, T b_x, T b_y, T b_z)     : a(a),		 b(b_x, b_y, b_z) {} \
Z_CT(CPP11) Line3D(const XYZ<T> &a, T b_xyz)		     : a(a),		 b(b_xyz)	  {} \
Z_CT(CPP11) Line3D(T a_x, T a_y, T a_z, const XYZ<T> &b)     : a(a_x, a_y, a_z), b(b)		  {} \
...
Z_CT(CPP11) Line3D(T a_xyz, const XYZ<T> &b)		     : a(a_xyz),	 b(b)		  {} \
Z_CT(CPP11) Line3D(const XYZ<T> &b)			     : a(T(0)),		 b(b)		  {} \
...
Z_INLINE void swap(Line3D &line)								\
	{Zeta::swap<Base>((Base *)this, (Base *)&line);}					\
												\
												\
Z_CT(CPP11) XYZ<T> segment_center() const							\
	{return a.middle(b);}
...
namespace Zeta {
	...
	template <class T> struct Line3D<T, Z_NUMBER_SET_R> {
		...
		Z_CT(CPP11) XYZ<T> segment_lerp(T alpha) const
