...
#define Z_IMPLEMENTATION_APPLE_CONSTRUCTOR(Prefix)   \
...
#	define Z_IMPLEMENTATION_CG_GEOMETRY	       \
		Z_IMPLEMENTATION_APPLE_CONSTRUCTOR(CG) \
		Z_IMPLEMENTATION_APPLE_OPERATOR	  (CG)
...
#		define Z_IMPLEMENTATION_NS_GEOMETRY	       \
			Z_IMPLEMENTATION_APPLE_CONSTRUCTOR(NS) \
			Z_IMPLEMENTATION_APPLE_OPERATOR	  (NS)
...
Z_INLINE Rectangle() Z_DEFAULTED({})								\
												\
												\
Z_CT(CPP11) Rectangle(const XY<T> &point, const XY<T> &size)					\
: point(point), size(size) {}									\
												\
												\
Z_CT(CPP11) Rectangle(const XY<T> &point, T size_x, T size_y)					\
: point(point), size(size_x, size_y) {}								\
												\
												\
Z_CT(CPP11) Rectangle(const XY<T> &point, T size_xy)						\
: point(point), size(size_xy) {}								\
												\
												\
Z_CT(CPP11) Rectangle(T point_x, T point_y, const XY<T> &size)					\
...
: point(point_x, point_y), size(size_xy) {}							\
												\
												\
Z_CT(CPP11) Rectangle(T point_xy, const XY<T> &size)						\
: point(point_xy), size(size) {}								\
												\
												\
Z_CT(CPP11) Rectangle(const XY<T> &size)							\
: point(T(0)), size(size) {}									\
												\
												\
...
: point(rectangle.point), size(rectangle.size) {}						\
												\
												\
static Z_CT(CPP14) Rectangle from_vertices(const XY<T> &a, const XY<T> &b)			\
	{											\
...
Z_INLINE Rectangle &operator |=(const Rectangle &rhs) {return *this = *this | rhs;}		\
												\
Z_CT(CPP11) Rectangle operator +(const XY<T> &rhs) const {return Rectangle(point + rhs, size);} \
Z_CT(CPP11) Rectangle operator -(const XY<T> &rhs) const {return Rectangle(point - rhs, size);} \
Z_CT(CPP11) Rectangle operator *(const XY<T> &rhs) const {return Rectangle(point * rhs, size);} \
Z_CT(CPP11) Rectangle operator /(const XY<T> &rhs) const {return Rectangle(point / rhs, size);} \
												\
Z_INLINE Rectangle &operator +=(const XY<T> &rhs) {return *this = *this + rhs;}			\
Z_INLINE Rectangle &operator -=(const XY<T> &rhs) {return *this = *this - rhs;}			\
Z_INLINE Rectangle &operator *=(const XY<T> &rhs) {return *this = *this * rhs;}			\
Z_INLINE Rectangle &operator /=(const XY<T> &rhs) {return *this = *this / rhs;}			\
												\
Z_CT(CPP11) Rectangle operator +(T rhs) const {return XY<T>(point + rhs, size);}		\
Z_CT(CPP11) Rectangle operator -(T rhs) const {return XY<T>(point - rhs, size);}		\
Z_CT(CPP11) Rectangle operator *(T rhs) const {return XY<T>(point * rhs, size);}		\
Z_CT(CPP11) Rectangle operator /(T rhs) const {return XY<T>(point / rhs, size);}		\
												\
...
Z_IMPLEMENTATION_COCOS2D_X									\
												\
												\
Z_CT(CPP11) Rectangle align_in_bottom_center(const XY<T> &size) const				\
	{return Rectangle(this->point.x + (this->size.x - size.x) / T(2), this->point.y, size);}\
												\
												\
Z_CT(CPP11) Rectangle align_in_bottom_left(const XY<T> &size) const				\
	{return Rectangle(this->point, size);}							\
												\
												\
Z_CT(CPP11) Rectangle align_in_bottom_right(const XY<T> &size) const				\
	{return Rectangle(this->point.x + this->size.x - size.x, this->point.y, size);}		\
												\
												\
Z_CT(CPP11) Rectangle align_in_center(const XY<T> &size) const					\
	{return Rectangle(this->point + (this->size - size) / T(2), size);}			\
												\
												\
Z_CT(CPP11) Rectangle align_in_center_left(const XY<T> &size) const				\
	{return Rectangle(this->point.x, this->point.y + (this->size.y - size.y) / T(2), size);}\
												\
												\
Z_CT(CPP11) Rectangle align_in_center_right(const XY<T> &size) const				\
	{											\
	return Rectangle									\
		(this->point.x +  this->size.x - size.x,					\
		 ...
		 size);										\
												...
												\
												\
Z_CT(CPP11) Rectangle align_in_top_center(const XY<T> &size) const				\
	{											\
	return Rectangle									\
		(this->point.x + (this->size.x - size.x) / T(2),				\
		 ...
		 size);										\
												...
												\
												\
Z_CT(CPP11) Rectangle align_in_top_left(const XY<T> &size) const				\
	{return Rectangle(this->point.x, this->point.y + this->size.y - size.y, size);}		\
												\
												\
Z_CT(CPP11) Rectangle align_in_top_right(const XY<T> &size) const				\
	{return Rectangle(this->point + this->size - size, size);}				\
												\
												\
Z_CT(CPP11) XY<T> bottom_center() const								\
...
Z_CT(CPP11) Rectangle bottom_half() const							\
	{return Rectangle(point, size.x, size.y / T(2));}					\
												\
												\
Z_CT(CPP11) XY<T> bottom_left() const								\
	{return point;}										\
												\
												\
Z_CT(CPP11) Rectangle bottom_left_quarter() const						\
	{return Rectangle(point, size / T(2));}							\
												\
												\
Z_CT(CPP11) XY<T> bottom_right() const								\
	{return XY<T>(point.x + size.x, point.y);}						\
												\
												\
Z_CT(CPP11) XY<T> center() const								\
	{return point + size / T(2);}								\
												\
												\
Z_CT(CPP11) XY<T> center_left() const								\
	{return XY<T>(point.x, point.y + size.y / T(2));}					\
												\
												\
Z_CT(CPP11) XY<T> center_right() const								\
...
/*Z_CT(CPP11) Boolean contains(const AABR<T> &aabr) const					\
	...
	return	aabr.a.x != aabr.b.x &&								\
		...
		aabr.b	 <= point + size;							\
												...
												\
												\
Z_CT(CPP11) Boolean contains(const XY<T> &point) const						\
...
Z_INLINE Rectangle &correct()									\
	...
	return *this;										\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_bottom_center(const XY<T> &size) const				\
	{											\
	...
	return Rectangle									\
		(this->point.x + (this->size.x - fitting_size.x) / T(2), this->point.y,		\
		 fitting_size);									\
												...
												\
												\
Z_CT(CPP11) Rectangle fit_in_bottom_left(const XY<T> &size) const				\
	{return Rectangle(this->point, size.fit(this->size));}					\
												\
												\
Z_CT(CPP14) Rectangle fit_in_bottom_right(const XY<T> &size) const				\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle									\
		(this->point.x + this->size.x - fitting_size.x, this->point.y, fitting_size);	\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_center(const XY<T> &size) const					\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle(this->point + (this->size - fitting_size) / T(2), fitting_size);	\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_center_left(const XY<T> &size) const				\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle									\
		(this->point.x, this->point.y + (this->size.y - fitting_size.y) / T(2),		\
		 fitting_size);									\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_center_right(const XY<T> &size) const				\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle									\
		(this->point.x +  this->size.x - fitting_size.x,				\
		 ...
		 fitting_size);									\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_top_center(const XY<T> &size) const				\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle									\
		(this->point.x + (this->size.x - fitting_size.x) / T(2),			\
		 ...
		 fitting_size);									\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_top_left(const XY<T> &size) const					\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle									\
		(this->point.x, this->point.y + this->size.y - fitting_size.y, fitting_size);	\
												...
												\
												\
Z_CT(CPP14) Rectangle fit_in_top_right(const XY<T> &size) const					\
	{											\
	XY<T> fitting_size = size.fit(this->size);						\
												\
	return Rectangle(this->point + this->size - fitting_size, fitting_size);		\
												...
												\
												\
Z_CT(CPP11) Rectangle grow_from_bottom_center(const XY<T> &delta) const				\
	{return Rectangle(point.x - delta.x / T(2), point.y, size + delta);}			\
												\
												\
Z_CT(CPP11) Rectangle grow_from_bottom_left(const XY<T> &delta) const				\
	{return Rectangle(point, size + delta);}						\
												\
												\
Z_CT(CPP11) Rectangle grow_from_bottom_right(const XY<T> &delta) const				\
	{return Rectangle(point.x - delta.x, point.y, size + delta);}				\
												\
												\
Z_CT(CPP11) Rectangle grow_from_center(const XY<T> &delta) const				\
	{return Rectangle(point - delta / T(2), size + delta);}					\
												\
												\
Z_CT(CPP11) Rectangle grow_from_center_left(const XY<T> &delta) const				\
	{return Rectangle(point.x, point.y - delta.y / T(2), size + delta);}			\
												\
												\
Z_CT(CPP11) Rectangle grow_from_center_right(const XY<T> &delta) const				\
	{return Rectangle(point.x - delta.x, point.y - delta.y / T(2), size + delta);}		\
												\
												\
Z_CT(CPP11) Rectangle grow_from_top_center(const XY<T> &delta) const				\
	{return Rectangle(point.x - delta.x / T(2), point.y - delta.y, size + delta);}		\
												\
												\
Z_CT(CPP11) Rectangle grow_from_top_left(const XY<T> &delta) const				\
	{return Rectangle(point.x, point.y - delta.y, size + delta);}				\
												\
												\
Z_CT(CPP11) Rectangle grow_from_top_right(const XY<T> &delta) const				\
...
Z_CT(CPP11) T minimum_y()									\
	const {return point.y;}									\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_bottom_center(const XY<T> &delta) const			\
	{return Rectangle(point.x + delta.x / T(2), point.y, size - delta);}			\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_bottom_left(const XY<T> &delta) const				\
	{return Rectangle(point, size - delta);}						\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_bottom_right(const XY<T> &delta) const			\
	{return Rectangle(point.x + delta.x, point.y, size - delta);}				\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_center(const XY<T> &delta) const				\
	{return Rectangle(point + delta / T(2), size - delta);}					\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_center_left(const XY<T> &delta) const				\
	{return Rectangle(point.x, point.y + delta.y / T(2), size - delta);}			\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_center_right(const XY<T> &delta) const			\
	{return Rectangle(point.x + delta.x, point.y + delta.y / T(2), size - delta);}		\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_top_center(const XY<T> &delta) const				\
	{return Rectangle(point.x + delta.x / T(2), point.y + delta.y, size - delta);}		\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_top_left(const XY<T> &delta) const				\
	{return Rectangle(point.x, point.y + delta.y, size - delta);}				\
												\
												\
Z_CT(CPP11) Rectangle shrink_from_top_right(const XY<T> &delta) const				\
	{return Rectangle(point + delta, size - delta);}					\
												\
												\
...
Z_CT(CPP11) XY<T> top_center() const								\
	{return XY<T>(point.x + size.x / T(2), point.y + size.y);}				\
												\
												\
Z_CT(CPP11) XY<T> top_left() const								\
	{return XY<T>(point.x, point.y + size.y);}						\
												\
												\
Z_CT(CPP11) XY<T> top_right() const								\
	{return point + size;}
...
namespace Zeta {
	...
	template <class T> struct Rectangle<T, Z_NUMBER_SET_R> {
		...
		Z_CT(CPP11) XY<T> absolute_point_to_unit(const XY<T> &point) const
		...
		Z_CT(CPP11) XY<T> unit_point_to_absolute(const XY<T> &point) const
			{return point * size + point;}
	};
}


#undef Z_IMPLEMENTATION_APPLE_CONSTRUCTOR
#undef Z_IMPLEMENTATION_APPLE_OPERATOR
