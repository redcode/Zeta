...
#ifndef Z_classes_Circle_HPP
#define Z_classes_Circle_HPP
...
namespace Zeta {template <class T> struct Circle {
	...
	Z_CT(CPP11) Circle(const XY<T> &point, T radius) : point(point), radius(radius) {}
	...
	Z_CT(CPP11) Boolean operator ==(const Circle &circle) const
		{
		return	point.x == circle.point.x && point.y == circle.point.y &&
