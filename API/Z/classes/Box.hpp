...
#ifndef _Z_classes_Box_HPP_
#define _Z_classes_Box_HPP_
...
Z_INLINE Box() Z_DEFAULTED({})								\
											\
											\
Z_CT(CPP11) Box(const XYZ<T> &point, const XYZ<T> &size)				\
: point(point), size(size) {}								\
											\
											\
Z_CT(CPP11) Box(const XYZ<T> &point, T size_x, T size_y, T size_z)			\
: point(point), size(size_x, size_y, size_z) {}						\
											\
											\
Z_CT(CPP11) Box(const XYZ<T> &point, T size_xyz)					\
: point(point), size(size_xyz) {}							\
											\
											\
Z_CT(CPP11) Box(T point_x, T point_y, T point_z, const XYZ<T> &size)			\
...
: point(point_x, point_y, point_z), size(size_xyz) {}					\
											\
											\
Z_CT(CPP11) Box(T point_xyz, const XYZ<T> &size)					\
: point(point_xyz), size(size) {}							\
											\
											\
Z_CT(CPP11) Box(const XYZ<T> &size)							\
: point(T(0)), size(size) {}								\
											\
											\
...
: point(T(0)), size(size_xyz) {}							\
											\
											\
static Z_CT(CPP14) Box from_vertices(const XYZ<T> &a, const XYZ<T> &b)			\
	{										\
	XYZ<T> minimum = a.minimum(b);							\
											\
...
Z_INLINE Box &operator |=(const Box &rhs) {return *this = *this | rhs;}			\
											\
Z_CT(CPP11) Box operator +(const XYZ<T> &rhs) const {return Box(point + rhs, size);}	\
Z_CT(CPP11) Box operator -(const XYZ<T> &rhs) const {return Box(point - rhs, size);}	\
Z_CT(CPP11) Box operator *(const XYZ<T> &rhs) const {return Box(point * rhs, size);}	\
Z_CT(CPP11) Box operator /(const XYZ<T> &rhs) const {return Box(point / rhs, size);}	\
											\
Z_INLINE Box &operator +=(const XYZ<T> &rhs) {return *this = *this + rhs;}		\
Z_INLINE Box &operator -=(const XYZ<T> &rhs) {return *this = *this - rhs;}		\
Z_INLINE Box &operator *=(const XYZ<T> &rhs) {return *this = *this * rhs;}		\
Z_INLINE Box &operator /=(const XYZ<T> &rhs) {return *this = *this / rhs;}		\
											\
Z_CT(CPP11) Box operator +(T rhs) const {return XYZ<T>(point + rhs, size);}		\
Z_CT(CPP11) Box operator -(T rhs) const {return XYZ<T>(point - rhs, size);}		\
Z_CT(CPP11) Box operator *(T rhs) const {return XYZ<T>(point * rhs, size);}		\
Z_CT(CPP11) Box operator /(T rhs) const {return XYZ<T>(point / rhs, size);}		\
											\
...
/*Z_CT(CPP11) Boolean contains(const AABB<T> &aabb) const				\
	...
	return	aabb.a.x != aabb.b.x && aabb.a.y != aabb.b.y && aabb.a.z != aabb.b.z &&	\
		aabb.a >= point && aabb.b <= point + size;				\
											...
											\
											\
Z_CT(CPP11) Boolean contains(const XYZ<T> &point) const					\
...
namespace Zeta {
	...
	template <class T> struct Box<T, Z_NUMBER_SET_R> {
		...
		Z_CT(CPP11) XYZ<T> absolute_point_to_unit(const XYZ<T> &point) const
		...
		Z_CT(CPP11) XYZ<T> unit_point_to_absolute(const XYZ<T> &point) const
