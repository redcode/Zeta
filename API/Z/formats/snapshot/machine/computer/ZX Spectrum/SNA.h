...
| usually not matter; the game (or whatever) will have stack space that can be |
| used for this. However, if this space is all in use when the snap is made,   |
| memory below the stack space will be corrupted. According to Rui Ribeiro,    |
| the effects of this can sometimes be avoided by replacing the corrupted      |
| bytes with zeros; e.g. take the PC from the, stack pointer, replace that     |
| word with 0000 and then increment SP. This worked with snapshots of Batman,  |
| Bounder and others which had been saved at critical points. Theoretically,   |
| this problem could cause a complete crash on a real Spectrum if the stack    |
| pointer happened to be at address 16384; the push would try and write to the |
| ROM.									       |
|									       |
...
| first 49179 bytes of the snapshot are otherwise exactly as described above. |
|									      |
| The third RAM bank saved {ram_bank_n} is always the one currently paged,    |
| even if this is page 5 or 2 - in this case, the bank is actually included   |
| twice. The remaining RAM banks {remaining_ram_banks} are saved in ascending |
| order - e.g. if RAM bank 4 is paged in, the snapshot is made up of banks 5, |
| 2 and 4 to start with, and banks 0, 1, 3, 6 and 7 afterwards. If RAM bank 5 |
| is paged in, the snapshot is made up of banks 5, 2 and 5 again, followed by |
...
Z_DEFINE_PACKED_STRUCTURE_BEGIN {
	...
	zuint8	im;
	zuint8	border_color;
	zuint8	ram_bank_5[16384];
	zuint8	ram_bank_2[16384];
	zuint8	ram_bank_n[16384];
	zuint16	pc;
